...
class TestConvertUtils(unittest.TestCase):
    ...
    def test_to_int_equal(self):
        self.assertEqual(expr.ConvertUtils.to_int('123'), 123, "Elements should be different")

    def test_to_int_non_equal(self):
        self.assertNotEqual(expr.ConvertUtils.to_int('321'), 123, "Elements should be different")
    ...
    def test_complex_tag_true(self):
        code = expr.XmlReader(complex_xml).parse()
        result = 'true' in code[0]
        self.assertEqual(result,True, "Complex tag missed")
